...
for line in sys.stdin:
    ...
    if len(parts) == 1:
        category_news = parts[0]
    else:
        news = {"news": parts[0], "category": parts[1], "reliability": float(parts[2])}
        all_news.append(news)

sort_news = sorted(all_news, key=lambda x: (x["reliability"], x["news"]))
for news in sort_news:
    if news["category"] == category_news:
        print(news["news"])
